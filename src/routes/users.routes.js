...
router.get('/logout', (req, res, next) => {
    req.logout((err) => {
        if (err) {
            return next(err); // Maneja errores durante el logout
        }
        req.flash('success_msg', 'Sesión cerrada'); // Asegúrate de que `connect-flash` esté configurado
        res.redirect('/users/signin'); // Redirige al formulario de inicio de sesión
    });
});
